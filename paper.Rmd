...
tools, the task only gets harder.  For instance, few ecological
...
towards increasingly more explicit descriptions of how stochastiticity
...
Consider Levins' patch model, where $n$ individuals compete for a
...
proportional to the fraction of available patches, $1 - n/N$.  Figure
1 shows the results of two exact SSA simulations of this model with
...
Levins model).  Note that the dynamics for the average population
...
Note that like the equation for the mean, we have a dynamic equation
for the time-evolution of the variance in Eq \eqref{bd}.  We can
solve these equations at steady-state by setting both equations
zero. For our Levins' model, $b(n) = n (1 - \tfrac{n}{N})$ and $d(n) = en$,
...
emerges from our derivation above, has also been a subject of
...
environmental noise process relative to the timescale (auto-correlation)
...
measurements are effectively independent: auto-correlated noise is
all about _relative_ timescales.
...
@Schreiber2017].  *Observational error*: Observational error acts
...
eventually go extinct under demographic noise.  While extinction
is certainly a phenomenon that stands in stark contrast to the
long-term behavior of the corresponding deterministic equation,
this result is not particularly useful: "eventually" could be longer
than the existence of the planet. Rather, it is a reminder that we
must be careful about how we describe the 'long-term' behavior of
a model.  In place of the mathematical notion of a stationary
distribution to describe the long-term behavior of a stochastic
model, analysis of demographic stochasticity focuses on the concept
of quasi-stationary distributions, which in many cases correspond
more closely to the equilibrium of their deterministic counterparts
[@Ovaskainen2010; @Schreiber2017].
...
cervisiae_ on a sucrose medium. Because the relevant proteins are
...
numerical simulation shown here.

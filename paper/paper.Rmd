...
Before we follow that path, I will highlight

I will review how a shift towards more mechanistic descriptions of the origins of noise provide a more nuianced view of when the canonical model does an does not hold in the context of both demographic and environmental stochasticity.  My approach will highlight two fundamental tools: an ability for efficient, exact simulation using @Gillespie1977's stochastic simulation algorithm (SSA), and the van Kampen system size expansion [@vanKampen1976].



which is rooted in the mathematical theorems of Thomas Kurtz [@Kurtz1970 & @Kurtz1971, @Kurtz1972 @Kurtz1978], in place of the more popular diffusion approximation derived by Taylor series [e.g. @Nisbet1982, @Lande2003; rooted in the earlier work of @Kramers1940 and @Moyal1949, see comparisons in @vanKampen2007 and @Gardiner2009].  
...
The @Gillespie1977 provides a simple and exact algorithm
...
\frac{\mathrm{d} \sigma^2 }{\mathrm{d} t}  &= 2 \left( \partial_x b(x)  - \partial_x d(x) \right) \sigma^2 + b(x) + d(x)  + \mathcal{O}(N^{-\tfrac{1}{2}}) \label{bd}
\end{align}


The dynamics for the average population
...
Note that the van Kampen expansion is not identical to the SDE 
postulated by the-so called canonical equation.  
As shown in Appendix B, the popular expansion

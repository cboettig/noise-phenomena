...
of these sources of noise more precise -- developments which have important
consquences for when and how the resulting models can be applied. 

the canonical model on the highly influential @Leigh1981, @Lande1993, @Foley1994
...
Stochastic differential equations of this form are so-called "Langevin equation," popular in the physics literature.  This notation is rather sloppy, as the standard calculus does not apply to stochastic equations.  The Langevin formulation, most popular in physics, is usually associated with a Stratonovich integral.  Mathematicians prefer to write stochastic differential equations as $\textrm{d} n = f(x) \textrm{d} t + g(x) \textrm{d} B_t$, where the differentials hint at the underlying definition that this is the sum of a classic Reimann integral (over $\textrm{d}t$) and a stochastic It\^o integral.  

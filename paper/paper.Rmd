...
<!-- FIXME look at Coulson, Bjornstead, Ovaskeinen and others and see how they define scope to focus on "population ecology" -->
...
from view by the nusiance of noise has slowly given way to recognition
...
mutations to continuously-valued traits in evolutionary dynamics [e.g. @Boettiger2009]. 


The @Gillespie1977 provides an exact algorithm
for simulating demographic stochasticity at an individual level.  
The algorithm is merely a simple and direct implemtentation of the master
equation, progressing in random step sizes deterimed
...
for writing simple and approximation-free[^2] stochastic ecological models. 

[^2]: that is, free from the approximation made by SDE models as we see in the
van Kampen example.  All models are, of course, only approixmations. 
...
with a spefiic example.  We will focus on the classic case of @Levins1969 patch model,
...
to use this same approach in alternate formuluations.  
...
to $\sqrt{N}$ as postulated by the canonical equation, Eq \eqref{canoical}.
...
the theorems of Kurtz at about the same time [@Kurtz1970 & @Kurtz1971, @Kurtz1972, in particular, see Thereom 4.4. @Kurtz1978].
...
experiences proportionally smaller fluctionations in population density.  This 
...
(i.e. averaged over an esemble of replicate realizations of the Gillespie simulations),
...
and deaths add randomness, they both add to the growth of varation observed, i.e.
$b(x) + d(x)$.  Second, we see that dynamics for the variance $\sigma_^2$ are *non-linear* whenever
...
roles of noise and nonlinearity in noise-driven phenomena such as stochastic
...
which will also prove useful later on.  Note this expression simply states that the equilbrium
...
the fluctuation-disspation theorem) and that correlations fall off exponentially at a rate given by this slope.   
We will see both autocorrelation and variance play similar roles to infer this eigenvalue when we 
...
or the review in @Ovaskeinen2010]. @Black2012 gives an excellent introduction 
...
mistakenly characterizing the van Kampen expansion as synonomous with the diffusion approximation.
...
a *non-linear* SDE of the form (e.g. @Ovaskeinen2010)
...
a more dubious double limit that often has no clear biological intepretation and
...
in the case of large deviations [see @Ovaskeinen2010], the van Kampen expansion
...
The van Kampen expansion can also be useful in illustrating how 
environmental noise enters a model as a separate process or dimension.
...
but unlike the canonical model, leads to explicitly autocorrelated rather 
than white noise (see derivation, Appendix B).  This result is intuitive:
for continuous time models, all fluctations are positively auto-correlated
by definition -- measure any continuous process at ever-closer intervals in
time and these measurements must converge. As the derivation makes it clear,
it is in fact the *relative* time scales of the environmental process
and population level process that really matter.  The degree of coupling
to environmental noise also depends on the derivative of the population 
dynamics with respect to the stochastically varying environmental variable:
the larger that derivative, the stronger the contribution of environmental
fluctuations to population fluctuations. For example, if extinction
rate $e$ in our Levins' model is allowed to vary
...
The importance of auto-correlation, has and continues to be a subject of
...
uncorrelated as a mathematical convenience. 
